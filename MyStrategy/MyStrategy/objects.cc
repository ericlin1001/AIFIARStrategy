...
void Object::reset(){
	position.SetVecPosition(0, 0);
  vector.SetVecPosition(0, 0);
  theta = 0.0;
}
// Constructor. 
...
void RobotPID::ToPositionPD(VecPosition target, double speed) {
  ...
  if (ToPositionPDTarget.GetDistanceTo(ToPositionPDOldTarget[0]) > 1) {
    ResetParamToPositionPD();//init the param
  }
 // ResetParamToPositionPD();//test
  
  Show("To target:");ToPositionPDTarget.toString();cout<<endl;
  Show("I'm here:");getPosition().toString();Show("theta:"<<this->getTheta()/PI*180.0<<"deg");
  
  // Get the distance between robot and target
  double direction=0;
 
  VecPosition pos_relative = ToPositionPDTarget - getPosition();
   direction=pos_relative.GetDirection();

  //direction=90.0/180.0*PI;
   Trace(direction/PI*180);
  
  ToPositionPDAngularDistance = VecPosition::NormalizeAngle(
      direction - getTheta());
  
  Trace(ToPositionPDAngularDistance);Trace(ToPositionPDAngularDistance/PI*180);

  ToPositionPDLinearDistance = pos_relative.GetX() * cos(getTheta())
      + pos_relative.GetY() * sin(getTheta());

  Show("ToPositionPDLinearDistance="<<ToPositionPDLinearDistance);
...
void RobotPID::TurnToAngle(double direction) {
  ...
  if (fabs(TurnToAngleAngularDirection-TurnToAngleOldAngularDirection[0])>0.001) {
   ...
   ResetParamTurnToAngle();//test
   Show("TurnToDirection:"<<TurnToAngleAngularDirection<<" "<<TurnToAngleAngularDirection/PI*180<<"deg");

  // Get angular distance
  TurnToAngleAngularDistance = VecPosition::NormalizeAngle(direction - getTheta());
  
  Show("Distance:"<<TurnToAngleAngularDistance);
  ...
  if (fabs(TurnToAngleAngularDistance) > PI / 2) {
   ...
   Show("after change Distance:"<<TurnToAngleAngularDistance<<" turn_back:"<<turn_back);
  ...
  Trace(TurnToAngleAngularParameter[P]);
  Trace(TurnToAngleAngularParameter[D]);
  Show("OldAngularDistance[0]:"<<TurnToAngleOldAngularDistance[0]);
  Show("angular_speed:"<<angular_speed);
