...
public:
	...
	void setPosition(double x, double y, CoordSystemT cs = CARTESIAN) {this->position.SetVecPosition(x, y, cs); }
	void setVector(const VecPosition &vec) { this->vector = vec; }
	void setVector(double x, double y, CoordSystemT cs = CARTESIAN) {vector.SetVecPosition(x, y, cs); }
	void setTheta(double theta) { this->theta = VecPosition::NormalizeAngle(theta); }
...
//
	void setLeftVelocity(double left_velocity) { this->leftVelocity = left_velocity; }
	void setRightVelocity(double right_velocity) {this->rightVelocity = right_velocity; }
...
private://auxiliary method:
	void setDesiredSpeed(double linearSpeed,double angularSpeed){
		Show("setDesiredSpeed linearSpeed:"<<linearSpeed<<" angularSpeed:"<<angularSpeed);
		setLeftVelocity(linearSpeed - 4.0* angularSpeed);
		setRightVelocity(linearSpeed +4.0 * angularSpeed);
		...
		*/
		...
		Show(" leftSpeed:"<<this->getLeftVelocity()<<" rightVelocity:"<<this->getRightVelocity());
	}
private:
