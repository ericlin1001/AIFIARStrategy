...
{   
	...
	//  
	********************************************************************************/  
	Show("TeamManager constructor ");
	Show("adding new AttackTeam() ");
	Show("create AttackTeam successfully!");
	addTeam (new AttackTeam ());   

	Show("adding new DefendTeam() ");
	addTeam (new DefendTeam ());  
	Show("end TeamManager constructor ");
}   
...
{   
	...
	// 初始化使用队形时所需要的状态条件   
	//   
	Show("team->initCondition()");
	if (team->initCondition () == false) {   
		//ASSERT (false);   
		;   
	}   
	Show("teams.push_back (team);");
...
TeamOrder::TeamOrder (TeamType type)   
	...
	, leaks ()   
{   
	Show("TeamOrder constuctor");
	for (int i = 0; i < PLAYERS_PER_SIDE; ++i)   
		roles[i] = RT_UNKNOWN; // 队形所需要的角色   
	Show("end TeamOrder constuctor");
}   
...
{   
	{   
		Show("AttackTeam::initRoles (void) ");
		//   
		// 为此队形安排角色   
		//   
...
{   
	{   
		Show("AttackTeam::initCondition (void) ");
		//   
		// 添加符合的区域   
		//   
		addBallArea (BA_3);   
