...
extern "C" STRATEGY_API void StrategyStep(Environment *Env) {
	...
	if(mode.nDEStartMode==FieldInfo::Match_State::PENALTY_KICK){
		if(Env->currentBall_pos_x > FieldInfo::get_field_length()/2 && Env->halfArea==FiedlInfo::)
		mode.nDEPenaltyDirection= FieldInfo::Penalty_State::DEFENSE;
		else  mode.nDEPenaltyDirection= FieldInfo::Penalty_State::DEFENSE;
	}
