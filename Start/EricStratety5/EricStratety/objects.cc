...
void RobotBasis::Update(const VecPosition &position, 
  ...
  old_vector_[0] = get_vector();
  old_theta_[0] = get_theta();
  old_left_velocity_[0] = left_velocity_;
  old_right_velocity_[0] = right_velocity_;
...
// Update robot information
void RobotPID::Update(const VecPosition &position, 
                      const VecPosition &vec, 
                      double theta) {
...
void RobotPID::TurnToAngle(double direction) {
  ...
  TurnToAngle_angular_distance_ = 
      VecPosition::NormalizeAngle(direction - get_theta());
  ...
  if (fabs(TurnToAngle_angular_distance_) > PI / 2) {
    turn_right = true;
    TurnToAngle_angular_distance_ = 
        (PI - fabs(TurnToAngle_angular_distance_)) *
        Maths::Sign(TurnToAngle_angular_distance_);
  }
  ...
  set_left_velocity(-ROBOT_INFO::HALF_LENGTH * angular_speed);
  set_right_velocity(ROBOT_INFO::HALF_LENGTH * angular_speed);
...
void RobotPID::SaveParamToPositionPD() {
  for (int i = 0; i < OLD_PID_DATA_NUM - 1; ++i) {
