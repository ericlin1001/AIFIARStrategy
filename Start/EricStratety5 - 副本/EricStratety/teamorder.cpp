...
// 队形管理   
//   
#define ADD addTeam (new AttackTeam ());
...
// public   
TeamOrder::TeamOrder (TeamNumber number):_number(number)   
	, _areas ()   
	, _speeds ()   
	, _directions ()   
	, _powers ()   
...
{   
	...
	{   
		...
		iterator it;
		for ( it = _areas.begin (); it != itend; ++it) {   
	...
	{   
		...
		iterator it;
		for ( it= _speeds.begin (); it != itend; ++it) {   
			if (*it == speed)   
				break;   
		}   
		if (it == itend)   
	...
	{   
		...
		iterator itend = _directions.end ();   
			iterator it;
		for ( it = _directions.begin (); it != itend; ++it) {   
			if (*it == direction)   
				break;   
		}   
		if (it == itend)   
	...
	{   
		...
		iterator itend = _powers.end ();   
			iterator it;
		for (it= _powers.begin (); it != itend; ++it) {   
			if (*it == power)   
				break;;   
		}   
		if (it == itend)   
	...
	{   
		...
		iterator itend = _leaks.end ();   
			iterator it;
		for ( it = _leaks.begin (); it != itend; ++it) {   
			if (*it == leak)   
				break;   
		}   
		if (it == itend)   
...
INITROLES_MACRO(TestTeam){
	addRole (RT_GOALKEEPER, 0);         // 守门员   
	addRole (RT_TEST, 1);    // 左上角的后卫   
	addRole (PT_DEFEND_LEFT_BOT, 2);    // 左下角的后卫   
	addRole (PT_DEFEND_RIGHT_TOP, 3);   // 右上角的后卫   
	addRole (PF_DEFEND_RIGHT_BOT, 4);   // 右下角的后卫  
	return true;
}
INITCONDITION_MACRO(TestTeam){
	this->addAllArea();
	//
	addAllSpeed ();                     // 添加所有速度   
	addAllDirection ();                 // 添加所有方向   
